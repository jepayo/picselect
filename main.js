/**
 * main_tobesusedingithub.js — GENERADO AUTOMATICAMENTE. No editar a mano.
 *
 * Fuente: APP/main.js + APP/backend-local.js + shared/core.js + shared/state.js
 * Regenerar con:  npm run build   (desde PICSELECT SERVER/)
 *
...
(() => {
  ...
  function isJpeg(it) {
    return /\.(jpe?g)$/i.test(it.name) || it.file?.type === "image/jpeg";
  }
  function isHeicName(name = "") {
    return /\.(heic|heif)$/i.test(name);
  }
  function isHeicFile(f) {
    return f?.type && /image\/hei[cf]/i.test(f.type) || isHeicName(f?.name);
  }
  async function analyzeImage(file) {
    ...
    if (!pixW || !pixH) {
      try {
        ...
        pixW = img.naturalWidth;
        pixH = img.naturalHeight;
      } catch (_) {
      }
    }
  ...
  function enterGpsEditMode(it, fig, onFlagsChange) {
    ...
    panel.innerHTML = `<input class="gps-ei gps-ei-lat" type="number" step="any" min="-90" max="90" placeholder="Lat"><input class="gps-ei gps-ei-lng" type="number" step="any" min="-180" max="180" placeholder="Lng"><div class="gps-edit-btns"><button class="gps-eo gps-eo-ok" title="Guardar (Enter)">✓ Guardar</button><button class="gps-eo gps-eo-x" title="Cancelar (Esc)">✕</button></div>`;
    ...
    panel.querySelector(".gps-eo-ok").addEventListener("click", (e) => {
      e.stopPropagation();
      tryCommit();
    });
    panel.querySelector(".gps-eo-x").addEventListener("click", (e) => {
      e.stopPropagation();
      exitGpsEditMode();
    });
  ...
  async function pumpHeicQueue() {
    ...
    heicActive++;
    try {
      job.resolve(await window.heic2any({ blob: job.file, toType: "image/jpeg", quality: 0.92 }));
    } catch (err) {
      job.reject(err);
    } finally {
      heicActive--;
  ...
  function createLocalBackend() {
    ...
    const heicListURLCache = /* @__PURE__ */ new Map();
    const heicOverlayURLCache = /* @__PURE__ */ new Map();
    const activeListURLs = /* @__PURE__ */ new Set();
    const activeOverlayURLs = /* @__PURE__ */ new Set();
    ...
    const backend = {
      ...
      async load(onProgress) {
        if (!("showDirectoryPicker" in window)) {
          alert("Tu navegador no soporta abrir carpetas.");
          return { items: null, scanning: false };
        }
      ...
      async rotateFile(it) {
        ...
        const key = it.relPath;
        const lu = heicListURLCache.get(key);
        if (lu) {
          URL.revokeObjectURL(lu);
          heicListURLCache.delete(key);
        }
        const ou = heicOverlayURLCache.get(key);
        if (ou) {
          URL.revokeObjectURL(ou);
          heicOverlayURLCache.delete(key);
        }
        await rotateJpeg90cw(it);
        return "";
      },
      onItemsLoaded(items, onProgress) {
        onProgress?.(`Listo: ${items.length} fotos`);
      },
      releaseURLs(level) {
        if (level === "list") {
      ...
      buildMetaFlagsExtra(fig, it) {
        if (it.lat != null) {
          fig.dataset.gpsLat = it.lat;
          fig.dataset.gpsLng = it.lng;
        } else {
          delete fig.dataset.gpsLat;
          delete fig.dataset.gpsLng;
        }
  ...
  function populateItems(photos, backend) {
    ...
    const starSet = /* @__PURE__ */ new Set();
    const ideaSet = /* @__PURE__ */ new Set();
    const lowSet = /* @__PURE__ */ new Set();
  ...
  function startApp(backend) {
    ...
    let allItems = [];
    let groups = [];
    let trashSet = /* @__PURE__ */ new Set();
    let starSet = /* @__PURE__ */ new Set();
    let ideaSet = /* @__PURE__ */ new Set();
    let lowSet = /* @__PURE__ */ new Set();
    let filterNoGps = false;
    let filterLowRes = false;
    let currentPage = 1;
    let currentMins = (() => {
      const t = parseInt(new URLSearchParams(location.search).get("t"), 10);
      return [2, 5, 10].includes(t) ? t : 5;
    })();
    let expressMode = false;
    const EXPRESS_MAX = 8;
    const BUCKETS_PER_PAGE = 3;
    let itemByKey = /* @__PURE__ */ new Map();
    let listMode = "bucket";
    ...
    function loadImg(img, it, level) {
      Promise.resolve(backend.getDisplayURL(it, level)).then((url) => {
        img.src = url;
      }).catch(() => {
        img.removeAttribute("src");
        img.alt = it.name + " (sin preview)";
      });
    }
    ...
    function filteredGroups() {
      ...
      return groups.map((g, i) => ({ g, absIndex: i })).filter(({ absIndex }) => visibleItemsForBucket(absIndex).length > 0);
    }
    function computeCounts2() {
      return computeCounts(allItems, trashSet, starSet, ideaSet, lowSet);
    }
    function updateCountersUI() {
      const { total, nStar, nTrash, nIdea, nPending, nLow, nNoGps } = computeCounts2();
    ...
    function populateFromData(photos) {
      resetState();
      ({ allItems, itemByKey, trashSet, starSet, ideaSet, lowSet } = populateItems(photos, backend));
      buildGroups2();
      currentPage = 1;
      updatePagerState();
      ...
      updateActionButtons();
      updateCountersUI();
      backend.onItemsLoaded?.(allItems, (msg) => {
    ...
    function resetState() {
      ...
      ideaSet.clear();
      lowSet.clear();
      filterNoGps = filterLowRes = false;
      updateFilterUI();
      currentPage = 1;
      listMode = "bucket";
      selBucketAbsIndex = 0;
      ...
      overlaySelIdx = 0;
      lastOpenAbsIndex = null;
      overlayEdgeIntent = null;
      moveBtn.disabled = true;
      moveIdeasBtn.disabled = true;
    ...
    function buildGroups2() {
      groups = buildGroups(allItems, showManaged, currentMins);
    }
    function totalPages() {
      return Math.max(1, Math.ceil(filteredGroups().length / BUCKETS_PER_PAGE));
    }
    ...
    function makeExclusive(toSet, o1, o2, key, markName) {
      if (toSet.has(key)) {
        toSet.delete(key);
        backend.persistMark(key, null);
      } else {
        toSet.add(key);
        o1.delete(key);
        o2.delete(key);
        backend.persistMark(key, markName);
      }
    }
    function toggleTrash(key, fig) {
      const it = itemByKey.get(key);
      if (isManaged(it)) return;
      makeExclusive(trashSet, starSet, ideaSet, key, "trash");
    ...
    function toggleStar(key, fig) {
      const it = itemByKey.get(key);
      if (isManaged(it)) return;
      makeExclusive(starSet, trashSet, ideaSet, key, "star");
      applyItemClasses(fig, it);
      updateActionButtons();
      updateCountersUI();
    }
    function toggleIdea(key, fig) {
      const it = itemByKey.get(key);
      if (isManaged(it)) return;
      makeExclusive(ideaSet, trashSet, starSet, key, "idea");
      applyItemClasses(fig, it);
      updateActionButtons();
      updateCountersUI();
    }
    function _modelHue(name) {
      let h = 0;
      for (let i = 0; i < name.length; i++) h = h * 31 + name.charCodeAt(i) & 65535;
      return h % 360;
    }
    function buildMetaFlags(fig, it) {
      ...
      if (!container) return;
      container.innerHTML = "";
      if (it.hdLevel === null || it.hdLevel === void 0) return;
    ...
    async function rotateSelected() {
      ...
      try {
        ...
        if (img) {
          Promise.resolve(backend.getDisplayURL(it, "overlay")).then((url) => {
            img.src = url + bust;
          });
        }
        if (isViewerOpen()) {
          Promise.resolve(backend.getDisplayURL(it, "overlay")).then((url) => {
            viewerImg.src = url + bust;
          });
        }
          ...
          if (btnRotate) {
            if (!isManaged(it)) {
              btnRotate.addEventListener("click", async (e) => {
                ...
                try {
                  const bust = await backend.rotateFile(it);
                  Promise.resolve(backend.getDisplayURL(it, "list")).then((url) => {
                    img.src = url + bust;
                  });
                  progressEl.textContent = `Rotada: ${it.name}`;
                } catch (err) {
    ...
    function getBucketSection(absIndex) {
      return groupsEl.querySelector(`.group[data-abs-index="${absIndex}"]`);
    }
    function getBucketGrid(absIndex) {
      const s = getBucketSection(absIndex);
      return s ? s.querySelector(".grid") : null;
    }
    ...
    function setSelectedPhoto(newIdx) {
      ...
      getBucketGrid(selBucketAbsIndex)?.children[selPhotoIndex]?.scrollIntoView({ block: "nearest" });
    }
    function clearAllListSelections() {
      groupsEl.querySelectorAll(".group").forEach((s) => {
        s.classList.remove("bucket-selected", "photo-mode");
        s.querySelectorAll(".item.selected").forEach((f) => f.classList.remove("selected"));
      });
    }
    ...
    function openBucket(index, { push = true } = {}) {
      ...
      push ? history.pushState({ overlay: true, g: currentOverlayIndex }, "", url) : history.replaceState({ overlay: true, g: currentOverlayIndex }, "", url);
    }
    function _hideOverlayInternal(skipFocusReturn = false) {
      ...
      if (!skipFocusReturn && returnIdx !== null) {
        listMode = "bucket";
        setSelectedBucket(returnIdx);
      }
      currentOverlayIndex = null;
      overlayEdgeIntent = null;
      updateCountersUI();
    ...
    function hideOverlay(skipFocusReturn = false) {
      if (skipFocusReturn || overlay.hidden) {
        _hideOverlayInternal(skipFocusReturn);
        return;
      }
      history.back();
    }
    function getOverlayFig(idx) {
      return overlayGrid.children[idx] || null;
    }
    ...
    function updateViewerImage() {
      ...
      viewerGps.textContent = it.lat != null && it.lng != null ? `📍 ${it.lat.toFixed(6)},  ${it.lng.toFixed(6)}` : "";
    ...
    function hasWriteSupport() {
      return backend.hasWriteSupport();
    }
    function countMovable(set) {
      let n = 0;
      for (const k of set) {
        const it = itemByKey.get(k);
        if (it && !isManaged(it)) n++;
      }
      return n;
    }
    ...
    async function moveMarkedTo(folderName, set) {
      ...
      for (const k of set) {
        const it = itemByKey.get(k);
        if (it && !isManaged(it)) movable.push(it);
      }
    ...
    document.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        case " ":
          e.preventDefault();
          openBucket(selBucketAbsIndex);
          break;
        case "ArrowUp":
          e.preventDefault();
          if (listMode === "bucket") {
            const _fg = filteredGroups();
            const _ci = _fg.findIndex(({ absIndex }) => absIndex === selBucketAbsIndex);
            if (_ci > 0) setSelectedBucket(_fg[_ci - 1].absIndex);
            else if (_ci < 0 && _fg.length > 0) setSelectedBucket(_fg[0].absIndex);
          } else {
            const cols = getGridCols(getBucketGrid(selBucketAbsIndex));
            if (vis.length) setSelectedPhoto(selPhotoIndex - cols);
          }
          break;
        case "ArrowDown":
          e.preventDefault();
          if (listMode === "bucket") {
            const _fg = filteredGroups();
            const _ci = _fg.findIndex(({ absIndex }) => absIndex === selBucketAbsIndex);
            if (_ci >= 0 && _ci < _fg.length - 1) setSelectedBucket(_fg[_ci + 1].absIndex);
            else if (_ci < 0 && _fg.length > 0) setSelectedBucket(_fg[_fg.length - 1].absIndex);
          } else {
            const cols = getGridCols(getBucketGrid(selBucketAbsIndex));
            if (vis.length) setSelectedPhoto(selPhotoIndex + cols);
          }
        ...
        case "X":
          if (listMode === "photo" && vis.length) {
            e.preventDefault();
            const it = vis[selPhotoIndex];
            if (!it || isManaged(it)) break;
    ...
    function overlayKeydown(e) {
      ...
      switch (e.key) {
        ...
        case " ":
          e.preventDefault();
          isViewerOpen() ? closeViewer() : openViewer();
          break;
        case "ArrowLeft":
          e.preventDefault();
          setOverlaySelected(overlaySelIdx - 1);
          break;
        case "ArrowRight":
          e.preventDefault();
          setOverlaySelected(overlaySelIdx + 1);
        ...
        case "ArrowDown": {
          ...
          else if (overlaySelIdx !== total - 1) {
            setOverlaySelected(total - 1);
            overlayEdgeIntent = "down";
          } else if (overlayEdgeIntent === "down" && currentOverlayIndex < groups.length - 1) {
            currentOverlayIndex++;
            overlaySelIdx = 0;
            overlayEdgeIntent = null;
            renderOverlay();
        ...
        case "ArrowUp": {
          ...
          else if (overlaySelIdx !== 0) {
            setOverlaySelected(0);
            overlayEdgeIntent = "up";
          } else if (overlayEdgeIntent === "up" && currentOverlayIndex > 0) {
            currentOverlayIndex--;
            overlaySelIdx = Math.max(0, visibleItemsForBucket(currentOverlayIndex).length - 1);
            overlayEdgeIntent = null;
            renderOverlay();
        ...
        case "X": {
          ...
          const it = vis[overlaySelIdx];
          if (!it || isManaged(it)) break;
          toggleTrash(keyFor(it), overlayGrid.children[overlaySelIdx]);
          syncOverlayAria(overlaySelIdx);
          updateViewerButtonsState();
          overlayEdgeIntent = null;
          break;
        }
        ...
        case "Z": {
          ...
          const it = vis[overlaySelIdx];
          if (!it || isManaged(it)) break;
          toggleStar(keyFor(it), overlayGrid.children[overlaySelIdx]);
          syncOverlayAria(overlaySelIdx);
          updateViewerButtonsState();
          overlayEdgeIntent = null;
          break;
        ...
        case "I": {
          ...
          const it = vis[overlaySelIdx];
          if (!it || isManaged(it)) break;
          toggleIdea(keyFor(it), overlayGrid.children[overlaySelIdx]);
          syncOverlayAria(overlaySelIdx);
          updateViewerButtonsState();
          overlayEdgeIntent = null;
          break;
    ...
    overlayPrev.addEventListener("click", () => {
      if (currentOverlayIndex > 0) {
        ...
        overlayEdgeIntent = null;
        renderOverlay();
        history.replaceState({ overlay: true, g: currentOverlayIndex }, "", buildURL(currentOverlayIndex));
      }
    });
    overlayNext.addEventListener("click", () => {
      if (currentOverlayIndex < groups.length - 1) {
        currentOverlayIndex++;
        overlaySelIdx = 0;
        overlayEdgeIntent = null;
        renderOverlay();
    ...
    viewerCopyPathBtn?.addEventListener("click", async (e) => {
      ...
      try {
        ...
        const o = viewerCopyPathBtn.textContent;
        viewerCopyPathBtn.textContent = "✓";
        setTimeout(() => {
          viewerCopyPathBtn.textContent = o;
        }, 1500);
      } catch (_) {
        progressEl.textContent = `Ruta: ${p}`;
      }
    });
    ...
    window.addEventListener("popstate", () => {
      if (!overlay.hidden) _hideOverlayInternal();
    });
    renderPage(currentPage);
    updateListSelectionUI();
    updateActionButtons();
    ...
    (function() {
      ...
      if (isDark) {
        toggleDarkBtn.classList.add("is-on");
        toggleDarkBtn.textContent = "☀️ Claro";
      }
